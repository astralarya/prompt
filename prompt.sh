...
then
      PS1_COMMAND='(command -v git && git rev-parse HEAD && git status -sb) || (command -v hg && hg identify --branch --id && hg status -q)'
fi
...
PS1='\[\e]0;\u@\h: \w\a\]\[\033[${PS1_USER_COLOR}m\]\u\[\033[${PS1_HOST_COLOR}m\]@\h\[\033[${PS1_PATH_COLOR}m\]:\w $(STATUS=$?; if [ $STATUS = 0 ]; then printf \[\033[%bm\]\(%b\) $PS1_STATUS_GOOD_COLOR $STATUS; else printf \[\033[%bm\]\(%b\) $PS1_STATUS_BAD_COLOR $STATUS; fi)$(if [ "$_JOBS" ]; then awk "{printf \"\[\033[0m\]\\n\[\033[${PS1_JOB_COLOR}m\]%s\",\$0}" <<<"$_JOBS"; fi)$(eval "$PS1_COMMAND" 2> /dev/null | awk "{printf \"\[\033[0m\]\\n\[\033[${PS1_COMMAND_COLOR}m\]%s\",\$0}")\[\033[0m\]\n\[\033[${PS1_PROMPT_COLOR}m\]\$\[\033[0m\] '
